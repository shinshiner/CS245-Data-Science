...
从以上的初步观察可以得知，Adult数据集存在数据缺失的情况（如第3行和10行的“?”），我对整个数据集进行统计后，发现数据集中共有3620个实例存在缺失值，而其中2799个实例的缺失值多于1个（表 \ref{tab:nan}）。同时，我发现分类目标（income）的部分值存在歧义，“<=50K.”与“<=50K”属于同类，却被赋上不同标签，在后续向量化过程中（\ref{sec:vector} 小节）我会进行处理。
...
在机器学习领域，用于分类的算法种类繁多，基本的分类算法包括了逻辑回归（Logistic Regression）、K近邻（KNN）、决策树、支持向量机（SVM）以及多层感知机（MLP）等。考虑到Adult数据集的特征维数并不高，且分类目标简单（二分类），我在本次作业中选择使用决策树、SVM以及MLP三种分类模型。除使用单独模型进行分类外，我尝试应用了模型集成的方法以提高相应的分类效果。
